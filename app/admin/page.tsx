...
import { GenerateRedoLinkButton } from "./generate-redo-link-button";
...
export default async function AdminPage() {
  ...
  // any customer's order page without going through the email round-trip.
  // Redo links are different — minted on-demand from the admin UI via the
  // GenerateRedoLinkButton, so each link is single-use.
  const orderLinks = new Map<string, string>();
  for (const o of orders) {
    const t = await signOrderToken(o.id);
    orderLinks.set(o.id, `/order/${o.id}?t=${encodeURIComponent(t)}`);
  }
            ...
            {orders.map((o) => (
              <tr key={o.id} className="border-t border-slate-100">
                ...
                <td className="px-4 py-2 whitespace-nowrap">
                  <a
                    href={orderLinks.get(o.id)}
                    target="_blank"
                    rel="noopener"
                    className="text-brand font-semibold hover:underline text-xs mr-4"
                  >
                    Voir →
                  </a>
                  <GenerateRedoLinkButton orderId={o.id} />
                </td>
              </tr>
            ))}
          </tbody>
